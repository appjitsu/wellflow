...
export class EnhancedRateLimiterService {
  ...
  async checkRateLimit(
    ...
    try {
      ...
      if (!isAllowed && currentCount >= config.requests && burstCount < (config.burstAllowance || 0)) {
        isAllowed = true;
        isBurstUsed = true;
  ...
  async detectAbuse(
    ...
    if (maxEndpointRequests >= this.abusePatterns.endpointFlooding.threshold) {
      patterns.push(`Endpoint flooding: ${maxEndpointRequests} requests to same endpoint`);
      riskScore += 0.4;
    }
    ...
    if (uniqueEndpoints >= this.abusePatterns.scatterRequests.uniqueEndpoints &&
        totalRequests >= this.abusePatterns.scatterRequests.threshold) {
      patterns.push(`Scatter requests: ${uniqueEndpoints} unique endpoints in ${totalRequests} requests`);
      riskScore += 0.3;
    }

    // Check suspicious user agents
    const suspiciousUA = recentRequests.some(req =>
      this.abusePatterns.suspiciousUserAgent.patterns.some(pattern => pattern.test(req.userAgent))
    );
    if (suspiciousUA) {
    ...
    const authFailures = recentRequests.filter(req =>
      req.endpoint.includes('/auth') && req.statusCode === 401
    ).length;
    if (authFailures >= this.abusePatterns.authFailures.threshold) {
      patterns.push(`High authentication failure rate: ${authFailures} failures`);
      riskScore += 0.6;
    }

    // Check oversized payloads
    const oversizedRequests = recentRequests.filter(req =>
      req.responseTime > 5000 // Slow responses might indicate large processing
    ).length;
    if (oversizedRequests > totalRequests * 0.3) {
      patterns.push('High number of slow requests (possible oversized payloads)');
      riskScore += 0.2;
    }

    // Check off-hours activity
    const currentHour = new Date().getHours();
    const isOffHours = this.abusePatterns.offHoursActivity.hours.includes(currentHour);
  ...
  async resetUserLimits(userId: string): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to reset rate limits for user ${userId}:`, error);
    }
  }
   ...
   */
  async getUserStatus(userId: string, tier: UserTier): Promise<{
    currentUsage: number;
    limit: number;
    remaining: number;
    resetTime: Date;
    ...
    try {
      ...
      return {
        ...
        remaining: Math.max(0, config.requests + (config.burstAllowance || 0) - currentUsage - burstUsed),
