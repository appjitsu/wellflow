...
@Injectable()
export class EnhancedEventBusService extends EventBus implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(EnhancedEventBusService.name);
  private eventStream$ = new Subject<{ event: IEvent; metadata: EventMetadata }>();
  ...
  async replayEvents(
    ...
    this.logger.log(`Event replay requested from ${fromTimestamp.toISOString()} for types: ${eventTypes?.join(', ') || 'all'}`);
  }

  /**
   * Publish events in batch for better performance
   */
  async publishBatch(events: IEvent[], source: string = 'system'): Promise<void> {
  ...
  private setupEventStreamProcessing(): void {
    ...
    const loggingSubscription = this.eventStream$
      .pipe(
        filter(({ event }) => this.shouldLogEvent(event)),
      )
      .subscribe({
        next: ({ event, metadata }) => {
  ...
  private setupMetricsCollection(): void {
    ...
    const metricsSubscription = this.eventStream$
      ...
      .subscribe({
        next: (events) => {
          this.logger.debug(`Processed ${events.length} events in the last minute`);
  ...
  private updateEventMetrics(eventType: string, action: 'published' | 'processed' | 'failed'): void {
