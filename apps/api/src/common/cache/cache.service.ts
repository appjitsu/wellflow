...
export class CacheService implements ICache {
  ...
  async getStats(): Promise<CacheStats> {
    ...
    const memory = memoryStats.status === 'fulfilled' ? memoryStats.value : null;
  ...
  async getWithSlidingExpiration<T>(
    ...
    if (value !== null) {
      ...
      if (this.memoryCache) {
        await this.memoryCache.set(key, value, { ttl: Math.min(ttlSeconds, 300) });
      }
    }
  ...
  private calculateCombinedHitRate(memory: CacheStats | null, redis: CacheStats | null): number {
