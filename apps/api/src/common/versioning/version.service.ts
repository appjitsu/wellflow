  ...
  checkCompatibility(requestedVersion: string): VersionCompatibility {
    ...
    if (!requested) {
      return {
        ...
        isCompatible: false,
        warnings: [
          `Version ${requestedVersion} is not available. Using default version ${defaultVersion.version}.`,
        ],
    ...
    if (requested.isDeprecated) {
      ...
      if (requested.sunsetDate) {
        warnings.push(
          `This version will be sunset on ${requested.sunsetDate.toISOString().split('T')[0]}.`,
        );
      }
