...
export class VersionInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    // Check compatibility
    const compatibility = this.versionService.checkCompatibility(resolvedVersion);
    ...
    if (compatibility.warnings.length > 0) {
      response.setHeader('API-Warnings', JSON.stringify(compatibility.warnings));
    }

    if (compatibility.suggestedVersion) {
      response.setHeader('API-Suggested-Version', compatibility.suggestedVersion);
    }
    ...
    // Log warnings if any
    if (compatibility.warnings.length > 0) {
      this.logger.warn(`API Version warnings for ${endpoint}:`, compatibility.warnings);
    }
