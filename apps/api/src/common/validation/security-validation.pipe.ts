...
interface SecurityViolation {
  type: 'sql_injection' | 'xss' | 'command_injection' | 'path_traversal' | 'malformed_data';
...
export class SecurityValidationPipe implements PipeTransform<any> {
  ...
  private readonly sqlInjectionPatterns = [
    { pattern: /(\bUNION\b|\bSELECT\b|\bINSERT\b|\bUPDATE\b|\bDELETE\b|\bDROP\b|\bCREATE\b|\bALTER\b|\bEXEC\b|\bEXECUTE\b)/i, severity: 'high' as const },
  ...
  private readonly xssPatterns = [
    { pattern: /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, severity: 'critical' as const },
    { pattern: /javascript:/gi, severity: 'high' as const },
    { pattern: /on\w+\s*=/gi, severity: 'high' as const },
    { pattern: /<iframe\b[^<]*(?:(?!<\/iframe>)<[^<]*)*<\/iframe>/gi, severity: 'high' as const },
    { pattern: /<object\b[^<]*(?:(?!<\/object>)<[^<]*)*<\/object>/gi, severity: 'high' as const },
    { pattern: /<embed\b[^<]*(?:(?!<\/embed>)<[^<]*)*<\/embed>/gi, severity: 'high' as const },
  ...
  private readonly commandInjectionPatterns = [
    ...
    { pattern: /\b(rm|del|format|shutdown|reboot|halt)\b/i, severity: 'critical' as const }, // Dangerous commands
  ...
  private readonly malformedDataPatterns = [
    ...
    { pattern: /[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/, severity: 'medium' as const }, // Control characters
  ];
  ...
  private async detectSecurityViolations(value: any, metadata: ArgumentMetadata): Promise<SecurityViolation[]> {
    const violations: SecurityViolation[] = [];
    const dataString = JSON.stringify(value);

    // Check all patterns
    const allPatterns = [
      ...this.sqlInjectionPatterns.map(p => ({ ...p, type: 'sql_injection' as const })),
      ...this.xssPatterns.map(p => ({ ...p, type: 'xss' as const })),
      ...this.commandInjectionPatterns.map(p => ({ ...p, type: 'command_injection' as const })),
      ...this.pathTraversalPatterns.map(p => ({ ...p, type: 'path_traversal' as const })),
      ...this.malformedDataPatterns.map(p => ({ ...p, type: 'malformed_data' as const })),
    ];
  ...
  private traverseObject(obj: any, callback: (path: string, value: any) => void, path = ''): void {
  ...
  private async handleSecurityViolations(
    ...
    if (this.auditLogService) {
      const highSeverityViolations = violations.filter(v => ['high', 'critical'].includes(v.severity));
    ...
    violations.forEach(violation => {
      ...
      this.logger[logLevel](`Security violation detected: ${violation.description}`, {
        type: violation.type,
