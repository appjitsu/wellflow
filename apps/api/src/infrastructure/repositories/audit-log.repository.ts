...
import { eq, and, gte, lte, desc, asc, sql, count, or, like } from 'drizzle-orm';
import * as schema from '../../database/schema';
import { AuditLogRepository, AuditLogFilters, AuditLogPagination, AuditLogSearchResult } from '../../domain/repositories/audit-log.repository.interface';
import { AuditLog, AuditAction, AuditResourceType } from '../../domain/entities/audit-log.entity';
...
export class AuditLogRepositoryImpl implements AuditLogRepository {
  ...
  async save(auditLog: AuditLog): Promise<void> {
    const data = {
      ...
      oldValues: auditLog.getOldValues() ? JSON.stringify(auditLog.getOldValues()) : null,
      newValues: auditLog.getNewValues() ? JSON.stringify(auditLog.getNewValues()) : null,
      success: auditLog.getSuccess(),
      errorMessage: auditLog.getErrorMessage(),
      metadata: auditLog.getMetadata() ? JSON.stringify(auditLog.getMetadata()) : null,
  ...
  async findByUserId(userId: string, pagination: AuditLogPagination = {}): Promise<AuditLogSearchResult> {
  ...
  async findByOrganizationId(organizationId: string, pagination: AuditLogPagination = {}): Promise<AuditLogSearchResult> {
    const { page = 1, limit = 50, sortBy = 'timestamp', sortOrder = 'desc' } = pagination;
  ...
  async findByResource(resourceType: AuditResourceType, resourceId: string, pagination: AuditLogPagination = {}): Promise<AuditLogSearchResult> {
    const { page = 1, limit = 50, sortBy = 'timestamp', sortOrder = 'desc' } = pagination;
  ...
  async search(filters: AuditLogFilters, pagination: AuditLogPagination = {}): Promise<AuditLogSearchResult> {
    const { page = 1, limit = 50, sortBy = 'timestamp', sortOrder = 'desc' } = pagination;
    ...
    if (filters.ipAddress) conditions.push(eq(auditLogs.ipAddress, filters.ipAddress));
    if (filters.requestId) conditions.push(eq(auditLogs.requestId, filters.requestId));
    if (filters.endpoint) conditions.push(like(auditLogs.endpoint, `%${filters.endpoint}%`));
  ...
  async getStatistics(filters?: { organizationId?: string; startDate?: Date; endDate?: Date }): Promise<{
    totalLogs: number;
    ...
    const [statsResult, actionsResult, resourcesResult, usersResult, recentResult] = await Promise.all([
      this.db
        .select({
          total: count(),
    ...
    const resourcesByType = resourcesResult.reduce((acc, row) => {
