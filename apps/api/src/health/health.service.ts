...
export class HealthCheckService {
  ...
  async checkHealth(): Promise<HealthCheckResult> {
    ...
    checks.forEach((result, index) => {
      const checkName = ['database', 'redis', 'circuit_breaker', 'retry', 'memory', 'external_services'][index];
    ...
    const allHealthy = Object.values(checkResults).every(check => check.status === 'healthy');
  ...
  async checkExternalServices(): Promise<any> {
    ...
    try {
      ...
      const openBreakers = circuitBreakerMetrics.filter(metric => metric.state === 'OPEN');
  ...
  private async checkCircuitBreakerHealth(): Promise<any> {
    ...
    try {
      ...
      const unhealthyBreakers = metrics.filter(m => m.state === 'OPEN' && m.failureCount > 5);
  ...
  private async checkRequiredServices(): Promise<boolean> {
    ...
    try {
      await this.auditLogService.logSystemAction('HEALTH_CHECK', 'system-health', true);
      return true;
    } catch {
      return false;
    }
