...
export class CircuitBreakerHealthIndicator extends HealthIndicator {
  ...
  async isHealthy(key: string): Promise<HealthIndicatorResult> {
    try {
      ...
      const unhealthyBreakers = metrics.filter(metric =>
        metric.state === 'OPEN' && metric.failureCount > 10
      );

      const openBreakers = metrics.filter(metric => metric.state === 'OPEN');
      const halfOpenBreakers = metrics.filter(metric => metric.state === 'HALF_OPEN');
