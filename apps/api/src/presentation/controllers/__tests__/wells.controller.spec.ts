...
describe('WellsController', () => {
  ...
  describe('deleteWell', () => {
    it('should delete well successfully', async () => {
      const wellId = 'well-123';
      ...
      const result = await controller.deleteWell(wellId, mockRequest);

      expect(commandBus.execute).toHaveBeenCalledWith(
        expect.any(DeleteWellCommand),
      );
      expect(result).toBeUndefined();
    });

    it('should delete well with system user when user is undefined', async () => {
      const wellId = 'well-123';

      mockCommandBus.execute.mockResolvedValue(undefined);

      const result = await controller.deleteWell(wellId, {});
    ...
    it('should handle well not found for deletion', async () => {
      const wellId = 'non-existent-well';
      ...
      await expect(controller.deleteWell(wellId, mockRequest)).rejects.toThrow(
        'Well not found',
      );
    });

    it('should handle command execution errors during deletion', async () => {
      const wellId = 'well-123';
    ...
    it('should handle authorization failures during deletion', async () => {
