...
import { AuditLog, AuditAction, AuditResourceType, AuditMetadata } from '../../domain/entities/audit-log.entity';
...
export class AuditLogService {
   ...
   */
  setRequestContext(requestId: string, sessionId?: string, correlationId?: string): void {
  ...
  async logSuccess(
    ...
    await this.logAction(action, resourceType, resourceId, true, undefined, changes, metadata);
  }
  ...
  async logFailure(
    ...
    await this.logAction(action, resourceType, resourceId, false, errorMessage, changes, metadata);
  }
  ...
  async logRead(
    ...
    await this.logSuccess(AuditAction.READ, resourceType, resourceId, {}, metadata);
  }
  ...
  async logExecute(
    ...
    await this.logSuccess(AuditAction.EXECUTE, resourceType, resourceId, {}, metadata);
  }

  /**
   * Log authentication events
   */
  async logLogin(userId: string, success: boolean = true, errorMessage?: string, metadata?: AuditMetadata): Promise<void> {
    const tempContext = { ...this.context, userId };
    this.context = tempContext;

    if (success) {
      await this.logSuccess(AuditAction.LOGIN, AuditResourceType.USER, userId, {}, metadata);
    } else {
      await this.logFailure(AuditAction.LOGIN, AuditResourceType.USER, userId, errorMessage, {}, metadata);
    }
  }

  async logLogout(userId: string, metadata?: AuditMetadata): Promise<void> {
    await this.logSuccess(AuditAction.LOGOUT, AuditResourceType.USER, userId, {}, metadata);
  }
  ...
  async logExport(
    ...
    await this.logSuccess(AuditAction.EXPORT, resourceType, undefined, {}, metadata);
  }
  ...
  async logApprove(
    ...
    await this.logSuccess(AuditAction.APPROVE, resourceType, resourceId, {}, metadata);
  }
  ...
  async logSubmit(
    ...
    await this.logSuccess(AuditAction.SUBMIT, resourceType, resourceId, {}, metadata);
  }
