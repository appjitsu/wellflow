...
export const userQueries = {
  ...
  keys: {
    ...
    list: (filters: UserFilters) =>
      [...userQueries.keys.lists(), filters] as const,
  ...
  update: () => ({
    ...
    onSuccess: (updatedUser: User) => {
      ...
      queryClient.setQueryData(
        userQueries.keys.detail(updatedUser.id),
        updatedUser
      );
    ...
    (set, get) => ({
      ...
      addVisitedPage: (page: string) =>
        set((state) => ({
          lastVisitedPages: [
            page,
            ...state.lastVisitedPages.filter((p) => p !== page),
          ].slice(0, 5),
        })),
    }),
    {
...
const { setSelectedUsers, toggleUserSelection, clearSelection } =
  useUserStore();
