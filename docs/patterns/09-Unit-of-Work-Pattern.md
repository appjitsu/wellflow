...
export class VendorActivationService {
  ...
  async activateVendor(vendorId: string): Promise<void> {
    ...
    try {
      ...
      const contracts = await this.contractRepository.findByVendorId(
        vendor.getId(),
        transaction
      );
  ...
  async deactivateVendorWithContracts(vendorId: string): Promise<void> {
    ...
    try {
      ...
      const activeContracts =
        await this.contractRepository.findActiveByVendorId(
          vendor.getId(),
          transaction
        );
...
export class UnitOfWork implements IUnitOfWork {
  ...
  get vendorRepository(): IVendorRepository {
    if (!this._vendorRepository) {
      this._vendorRepository = this.vendorRepoFactory.create(
  ...
  get contractRepository(): IContractRepository {
    if (!this._contractRepository) {
      this._contractRepository = this.contractRepoFactory.create(
        this.transaction || this.db
      );
    }
  ...
  get auditRepository(): IAuditRepository {
    if (!this._auditRepository) {
      this._auditRepository = this.auditRepoFactory.create(
        this.transaction || this.db
      );
    }
  ...
  get paymentRepository(): IPaymentRepository {
    if (!this._paymentRepository) {
      this._paymentRepository = this.paymentRepoFactory.create(
        this.transaction || this.db
      );
    }
  ...
  get losRepository(): ILosRepository {
    if (!this._losRepository) {
      this._losRepository = this.losRepoFactory.create(
        this.transaction || this.db
      );
    }
  ...
  private getRepositoryForEntity(entity: Entity): any {
    ...
    throw new Error(
      `No repository found for entity type: ${entity.constructor.name}`
    );
  }
}
...
@Injectable()
export class VendorActivationHandler
  implements ICommandHandler<ActivateVendorCommand>
{
  constructor(
  ...
  async execute(command: ActivateVendorCommand): Promise<void> {
    ...
    // Related entities
    const contracts = await this.unitOfWork.contractRepository.findByVendorId(
      vendor.getId()
    );
...
export class FinalizeLosHandler implements ICommandHandler<FinalizeLosCommand> {
  ...
  async execute(command: FinalizeLosCommand): Promise<void> {
    const los = await this.unitOfWork.losRepository.findById(
      new LosId(command.losId)
    );
  ...
  private async processExpenseAllocations(
  ...
  private async updateVendorPayments(
    los: LeaseOperatingStatement
  ): Promise<void> {
...
```typescript
@Injectable()
export class BulkVendorUpdateHandler
  implements ICommandHandler<BulkUpdateVendorsCommand>
{
  constructor(private readonly unitOfWork: IUnitOfWork) {}

  async execute(
    command: BulkUpdateVendorsCommand
...
describe('UnitOfWork', () => {
  ...
  describe('change tracking', () => {
    it('should track new entities', () => {
      ...
      expect(
        unitOfWork['newObjects'].has(`Vendor:${vendor.getId().getValue()}`)
      ).toBe(true);
    });
    ...
    it('should track dirty entities', () => {
      ...
      expect(
        unitOfWork['dirtyObjects'].has(`Vendor:${vendor.getId().getValue()}`)
      ).toBe(true);
    });
  ...
  describe('commit', () => {
    ...
    it('should rollback on error', async () => {
      ...
      jest
        .spyOn(unitOfWork, 'vendorRepository', 'get')
        .mockReturnValue(mockVendorRepo as any);
...
describe('UnitOfWork Integration', () => {
  ...
  it('should handle complex business transaction', async () => {
    ...
    // Then
    const savedVendor = await unitOfWork.vendorRepository.findById(
      vendor.getId()
    );
    const savedContract = await unitOfWork.contractRepository.findById(
      contract.getId()
    );
  ...
  it('should rollback on constraint violation', async () => {
    ...
    // Verify rollback
    const foundVendor = await unitOfWork.vendorRepository.findByCode(
      new VendorCode('DUPLICATE')
    );
