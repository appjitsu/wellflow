  ...
  private updateAverageResponseTime(responseTime: number): void {
    if (this.stats.totalCalls === 1) {
      ...
      this.stats.averageResponseTime =
        (this.stats.averageResponseTime * (this.stats.totalCalls - 1) +
          responseTime) /
        this.stats.totalCalls;
    }
  }
...
export class ExternalIntegrationService {
  ...
  constructor(
    ...
    this.erpCircuitBreaker = this.circuitBreakerRegistry.getOrCreate(
      'ERP_SERVICE',
      {
        failureThreshold: 5,
    ...
    this.paymentCircuitBreaker = this.circuitBreakerRegistry.getOrCreate(
      'PAYMENT_SERVICE',
      {
        failureThreshold: 3,
  ...
  async syncVendorToERP(vendor: Vendor): Promise<void> {
    try {
      ...
      if (error instanceof CircuitBreakerOpenError) {
        ...
        console.warn(
          'ERP circuit breaker is open, queued vendor sync for later',
          {
            vendorId: vendor.getId().getValue(),
          }
        );
  ...
  async updatePaymentStatus(paymentId: string, status: string): Promise<void> {
    try {
      ...
      if (error instanceof CircuitBreakerOpenError) {
        ...
        console.warn(
          'Payment service circuit breaker is open, stored update for retry'
        );
        return;
      }
  ...
  async sendNotification(notification: NotificationData): Promise<void> {
    try {
      ...
      if (error instanceof CircuitBreakerOpenError) {
        ...
        console.warn(
          'Notification service circuit breaker is open, used fallback'
        );
        return;
      }
  ...
  private async storePaymentUpdateForRetry(
    paymentId: string,
    status: string
  ): Promise<void> {
  ...
  private async useFallbackNotification(
...
export class AdaptiveCircuitBreaker extends CircuitBreaker {
  ...
  private updateAdaptiveThresholds(): void {
    ...
    if (avgHistoricalFailureRate > 0.2) {
      ...
      this.adaptiveConfig.failureThreshold = Math.max(
        2,
        this.adaptiveConfig.failureThreshold - 1
      );
    } else if (avgHistoricalFailureRate < 0.05) {
      // Low historical failure rate - be less sensitive
      this.adaptiveConfig.failureThreshold = Math.min(
        10,
        this.adaptiveConfig.failureThreshold + 1
      );
    }
...
export class BulkheadService {
  ...
  constructor(circuitBreakerRegistry: CircuitBreakerRegistry) {
    ...
    this.vendorOperationsCircuitBreaker = circuitBreakerRegistry.getOrCreate(
      'VENDOR_OPERATIONS',
      {
        failureThreshold: 5,
  ...
  async collectMetrics(): Promise<void> {
    ...
    for (const [name, breaker] of circuitBreakers) {
      ...
      this.metricsService.gauge(
        `circuit_breaker.failure_rate.${name}`,
        stats.failureRate
      );
      this.metricsService.gauge(
        `circuit_breaker.total_calls.${name}`,
        stats.totalCalls
      );
      this.metricsService.gauge(
        `circuit_breaker.successful_calls.${name}`,
        stats.successfulCalls
      );
      this.metricsService.gauge(
        `circuit_breaker.failed_calls.${name}`,
        stats.failedCalls
      );
      this.metricsService.gauge(
  ...
  describe('OPEN state', () => {
    ...
    it('should move to HALF_OPEN after recovery timeout', async () => {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, config.recoveryTimeoutMs + 100)
      );
...
async function sendNotification(notification: NotificationData): Promise<void> {
  try {
    await circuitBreaker.execute(() =>
      externalNotificationService.send(notification)
    );
  } catch (error) {
    if (error instanceof CircuitBreakerOpenError) {
