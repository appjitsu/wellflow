...
export class VendorService {
  ...
  async getActiveVendorsByOrganization(orgId: string): Promise<VendorDto[]> {
    ...
    const results = await this.db
      ...
      .from(vendors)
      .where(
        and(eq(vendors.organizationId, orgId), eq(vendors.status, 'ACTIVE'))
      );
...
export interface IVendorRepository {
  ...
  existsByCodeInOrganization(
    code: VendorCode,
    organizationId: string
  ): Promise<boolean>;

  findVendorsForPayment(
    organizationId: string,
    minAmount: Money
  ): Promise<Vendor[]>;
...
export class VendorRepository implements IVendorRepository {
  ...
  async findActiveVendorsByOrganization(
    organizationId: string
  ): Promise<Vendor[]> {
    const results = await this.db
      .select()
  ...
  async findVendorsWithExpiredInsurance(
    organizationId: string
  ): Promise<Vendor[]> {
    const results = await this.db
      .select()
  ...
  async existsByCodeInOrganization(
    code: VendorCode,
    organizationId: string
  ): Promise<boolean> {
    const result = await this.db
  ...
  async findByOrganizationPaginated(
    ...
    const results = await this.db
      ...
      .where(eq(vendors.organizationId, organizationId))
      .orderBy(
        sortOrder === 'desc' ? desc(vendors[sortBy]) : asc(vendors[sortBy])
      )
      .limit(limit)
      .offset(offset);
  ...
  implements ICommandHandler<CreateVendorCommand>
{
  ...
  async execute(command: CreateVendorCommand): Promise<string> {
    ...
    if (
      await this.vendorRepository.existsByCodeInOrganization(
        vendorCode,
...
describe('VendorRepository', () => {
  ...
  describe('findActiveVendorsByOrganization', () => {
    it('should return active vendors for organization', async () => {
      ...
      // When
      const results =
        await repository.findActiveVendorsByOrganization(organizationId);
...
export interface IVendorRepository
  extends IVendorReadRepository,
    IVendorWriteRepository {}
```
