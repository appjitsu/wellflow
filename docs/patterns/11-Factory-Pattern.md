...
export class VendorCreationService {
  ...
  async createContractVendor(
    contractDto: CreateContractVendorDto
  ): Promise<Vendor> {
    ...
    await this.vendorRepository.save(vendor);
    await this.auditService.logContractVendorCreation(
      vendor.id,
      contractDto.userId
    );
...
export class StandardVendorFactory implements IVendorFactory {
  ...
  private async generateVendorCode(
    name: string,
...
export class ContractVendorFactory implements IVendorFactory {
  ...
  private async generateVendorCode(
    ...
    return await this.codeGenerator.generateUniqueCodeWithPrefix(
      'CTR',
      baseCode,
      organizationId
    );
...
export class VendorBuilder implements IVendorBuilder {
  ...
  private async validateData(): Promise<void> {
    ...
    const errors = await this.validator.validateCreationData(
      this.vendorData as VendorCreationData
    );
    if (errors.length > 0) {
      throw new VendorValidationError(errors);
    }
  ...
  private determineInitialStatus(): VendorStatus {
    return this.vendorData.contractInfo
      ? VendorStatus.ACTIVE
  ...
  private buildInsurance(): Insurance | undefined {
    ...
    return Insurance.create({
      ...
      coverageAmount: Money.fromAmount(
        this.vendorData.insurance.coverageAmount,
        'USD'
      ),
    });
  }
...
export class ConfigurableVendorFactory implements IVendorFactory {
  ...
  private async validateUsingConfiguration(
...
describe('StandardVendorFactory', () => {
  ...
  beforeEach(() => {
    ...
    mockCodeGenerator = {
      generateUniqueCode: jest
        .fn()
        .mockResolvedValue(new VendorCode('TEST-01')),
    };
    ...
    factory = new StandardVendorFactory(
      mockIdGenerator,
      mockCodeGenerator,
      mockValidator
    );
  ...
  describe('createVendor', () => {
    ...
    it('should throw error for invalid data', async () => {
      ...
      mockValidator.validateCreationData.mockResolvedValue([
        'Vendor name is required',
      ]);

      await expect(factory.createVendor(data)).rejects.toThrow(
...
export abstract class BaseFactory<T> {
  ...
  protected validateSupport(type: string): void {
    if (!this.supports(type)) {
      throw new Error(
        `${this.constructor.name} does not support type: ${type}`
      );
    }
  }
