...
export class RetryFactory {
  ...
  createExternalServiceRetry(serviceName: string): RetryExecutor {
    return new RetryExecutor({
      ...
      onRetry: (attempt, error) => {
        console.warn(
          `External service ${serviceName} failed, attempt ${attempt}`,
          {
            service: serviceName,
            error: error.message,
            type: error.constructor.name,
  ...
  private isRetryableServiceError(error: Error): boolean {
    ...
    return (
      this.isRetryableHttpError(error) || this.isRetryableDatabaseError(error)
    );
  }
}
...
export class PaymentService {
  constructor(
    private readonly retryCircuitBreakerService: RetryWithCircuitBreakerService
  ) {}
...
export class ApiService {
  ...
  async callRateLimitedApi(data: any): Promise<any> {
    const response = await this.httpService
      .get('/rate-limited-api', { params: data })
      .toPromise();
    return response.data;
  }
}
    ...
    outcomes.forEach((outcome, index) => {
      if (outcome.status === 'fulfilled') {
        ...
        if (result.success) {
          ...
          failures.push(
            new BulkOperationFailure(result.item, result.error, index)
          );
        }
      } else {
        failures.push(
          new BulkOperationFailure(items[index], outcome.reason, index)
        );
      }
    });
...
describe('RetryExecutor', () => {
  ...
  describe('failing operations', () => {
    it('should not retry non-retryable errors', async () => {
      ...
      await expect(retryExecutor.execute(operation)).rejects.toThrow(
        'non-retryable'
      );
      expect(operation).toHaveBeenCalledTimes(1);
    });
    ...
    it('should fail after max attempts', async () => {
      const operation = jest
        .fn()
        .mockRejectedValue(new Error('retry-able error'));
  ...
  describe('retry strategies', () => {
    it('should calculate exponential backoff delays correctly', () => {
      const delays = [1, 2, 3].map((attempt) =>
        retryExecutor['calculateDelay'](attempt)
      );
...
describe('ExternalApiService with Retry', () => {
  ...
  describe('syncVendorData', () => {
    it('should retry on failure and succeed', async () => {
      ...
      mockRetryFactory.createHttpRetry.mockReturnValue(
        mockRetryExecutor as any
      );
