...
export class VendorExportDto {
  ...
  static fromEntity(
    vendor: Vendor,
    aggregateData: VendorAggregateData
  ): VendorExportDto {
...
describe('VendorDtoMapper', () => {
  ...
  describe('toDto', () => {
    it('should map vendor entity to DTO correctly', () => {
      ...
      expect(dto.contactInfo.email).toBe(
...
export class CreateVendorDto {
  ...
  @Matches(/^[A-Za-z0-9_-]{3,20}$/, {
    message:
      'Vendor code must be 3-20 characters, alphanumeric with hyphens or underscores only',
  })
  readonly code: string;
}
```
