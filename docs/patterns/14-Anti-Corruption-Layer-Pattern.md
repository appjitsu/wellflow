...
export class ErpAdapter implements IErpAdapter {
  ...
  async updateVendor(vendor: Vendor): Promise<void> {
    try {
      const erpVendorRequest = this.translateVendorToErpFormat(vendor);
      await this.erpApiClient.updateVendor(
  ...
  async syncVendorStatus(
    vendorId: VendorId,
    status: VendorStatus
  ): Promise<void> {
    try {
      const erpStatus = this.translateStatusToErpFormat(status);
      await this.erpApiClient.updateVendorStatus(
        vendorId.getValue(),
        erpStatus
      );
    } catch (error) {
      ...
      throw new ErpIntegrationError(
        'Failed to sync vendor status with ERP',
        error
      );
    }
  }
...
export class BankingAdapter implements IBankingAdapter {
  ...
  async registerPayee(
    ...
    try {
      const bankingRequest = this.translateVendorToBankingFormat(
        vendor,
        bankingInfo
      );
      const response =
        await this.bankingApiClient.registerPayee(bankingRequest);
  ...
  async updatePayeeInfo(
    ...
    try {
      ...
      await this.bankingApiClient.updatePayee(
        vendor.getId().getValue(),
        bankingRequest
      );
    } catch (error) {
  ...
  private translateVendorToBankingFormat(
    ...
    return {
      ...
      account_type: this.translateAccountTypeToBanking(
        bankingInfo.getAccountType()
      ),
...
export class ComplianceAdapter implements IComplianceAdapter {
  ...
  async updateInsuranceRecord(vendor: Vendor): Promise<void> {
    try {
      ...
      throw new ComplianceIntegrationError(
  ...
  async checkComplianceStatus(vendorId: VendorId): Promise<ComplianceStatus> {
    try {
      const response = await this.complianceApiClient.getEntityStatus(
        vendorId.getValue()
      );
  ...
  private translateVendorToComplianceFormat(
    vendor: Vendor
    ...
    return {
      ...
      InsuranceInfo: insurance
        ? {
            ...
            CoverageTypes: this.translateCoverageTypesToCompliance(
              insurance.getCoverageTypes()
            ),
  ...
  private translateComplianceStatusFromExternal(
    externalStatus: any
  ): ComplianceStatus {
    ...
    const status =
      statusMapping[externalStatus.Status] || ComplianceStatus.UNKNOWN;
  ...
  private mapCoverageTypeToCompliance(
  ...
  private getMinimumCoverageAmount(
    coverageType: InsuranceCoverageType
  ): number {
    const minimums = {
...
export class VendorService {
  ...
  async updateVendorInsurance(
    command: UpdateVendorInsuranceCommand
  ): Promise<void> {
...
export class VendorExternalIntegrationHandler {
  ...
  async handleVendorInsuranceUpdated(
    ...
    try {
      ...
      console.error(
        'Failed to update compliance record after insurance update',
        error
      );
    }
  }
  ...
  implements IAntiCorruptionLayer<TDomain, TExternal>
{
  ...
  protected handleTranslationError(error: Error, context: string): never {
    throw new TranslationError(
      `Failed to translate ${context}: ${error.message}`,
      error
    );
  }
...
@Injectable()
export class VendorErpAntiCorruptionLayer extends BaseAntiCorruptionLayer<
  Vendor,
  ErpVendor
> {
...
export class ConfigurableAntiCorruptionLayer {
  ...
  toDomain<T>(entityType: string, externalData: any): T {
    ...
    if (!config) {
      throw new Error(
        `No ACL configuration found for entity type: ${entityType}`
      );
    }
    ...
    for (const mapping of config.fieldMappings) {
      const externalValue = this.getNestedValue(
        externalData,
      ...
      if (externalValue !== undefined && externalValue !== null) {
        ...
        if (mapping.defaultValue !== undefined) {
          ...
          throw new TranslationError(
            `Required field ${mapping.externalField} is missing`
          );
        }
      }
    }
    ...
    for (const [transformName, transform] of Object.entries(
...
describe('ErpAdapter', () => {
  ...
  describe('createVendor', () => {
    ...
    it('should handle ERP API errors gracefully', async () => {
      // Given
      const vendor = createTestVendor();
      mockErpApiClient.createVendor.mockRejectedValue(
        new Error('ERP service unavailable')
      );

      // When/Then
      await expect(erpAdapter.createVendor(vendor)).rejects.toThrow(
        ErpIntegrationError
      );
    });
  });
  ...
  describe('translation methods', () => {
    it('should translate vendor status correctly', () => {
      expect(
        erpAdapter['translateStatusToErpFormat'](VendorStatus.PENDING)
      ).toBe('PENDING_APPROVAL');
      expect(
        erpAdapter['translateStatusToErpFormat'](VendorStatus.ACTIVE)
      ).toBe('ACTIVE');
      expect(
        erpAdapter['translateStatusToErpFormat'](VendorStatus.BLOCKED)
      ).toBe('HOLD');
    });

    it('should translate payment terms correctly', () => {
      expect(
        erpAdapter['translatePaymentTermsToErp'](PaymentTerms.NET_15)
      ).toBe('NET15');
      expect(
        erpAdapter['translatePaymentTermsToErp'](PaymentTerms.NET_30)
      ).toBe('NET30');
...
describe('BankingAdapter', () => {
  ...
  describe('registerPayee', () => {
    ...
    it('should handle banking API errors', async () => {
      ...
      const vendor = createTestVendor();
      const bankingInfo = createTestBankingInfo();
      mockBankingApiClient.registerPayee.mockRejectedValue(
        new Error('Banking service error')
      );

      // When/Then
      await expect(
        bankingAdapter.registerPayee(vendor, bankingInfo)
      ).rejects.toThrow(BankingIntegrationError);
    });
  });
});
```
