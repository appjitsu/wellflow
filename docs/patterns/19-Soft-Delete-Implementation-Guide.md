...
```typescript
// Update: apps/api/src/infrastructure/database/schema/users.schema.ts
export const usersTable = pgTable('users', {
  ...
  role: userRoleEnum('role').notNull().default('pumper'),
...
// Create base soft delete interface
...
export class UsersService {
  ...
  async softDeleteUsersByOrganization(
...
export type DeletedUser = User &
  Required<Pick<User, 'deletedAt' | 'deletedBy'>>;
```
