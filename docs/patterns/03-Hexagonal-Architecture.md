...
class WellProductionService {
  ...
  async recordDailyProduction(wellId: string, data: any) {
    ...
    const well = await this.db.query('SELECT * FROM wells WHERE id = $1', [
      wellId,
    ]);
...
class Well {
  ...
  calculateNetProduction(grossProduction: Production): Production {
    ...
    return grossProduction.applyLossFactor(lossFactor);
  }
...
class RecordProductionUseCase {
  ...
  async execute(command: RecordProductionCommand): Promise<void> {
    ...
    await this.eventBus.publish(
      new ProductionRecordedEvent(well.id, netProduction)
    );
  }
...
class TexasRRCAdapter implements ICompliancePort {
  ...
  private mapToTexasFormat(
    well: Well,
    production: Production
  ): TexasProductionReport {
    return {
...
class EmailNotificationAdapter implements INotificationPort {
  ...
  async notifyProductionRecorded(
    well: Well,
    production: Production
  ): Promise<void> {
...
class ApplicationConfig {
  configure(container: DIContainer): void {
    ...
    // Repositories
    container
      .bind<IWellRepository>(TYPES.WellRepository)
      .to(PostgresWellRepository);
    ...
    container
      .bind<ICompliancePort>(TYPES.OklahomaCompliance)
      .to(OklahomaOCCAdapter);

    // Notifications
    container
      .bind<INotificationPort>(TYPES.NotificationPort)
