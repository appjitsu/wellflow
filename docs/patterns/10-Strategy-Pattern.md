...
export class AchPaymentStrategy implements IPaymentCalculationStrategy {
  ...
  supportsUrgency(urgency: PaymentUrgency): boolean {
    return [
      ...
      PaymentUrgency.EXPEDITED,
      PaymentUrgency.EMERGENCY,
    ].includes(urgency);
  }
}
...
export class WireTransferStrategy implements IPaymentCalculationStrategy {
  ...
  supportsUrgency(urgency: PaymentUrgency): boolean {
    return [PaymentUrgency.STANDARD, PaymentUrgency.EXPEDITED].includes(
      urgency
    );
  }
}
...
export class PaymentProcessingService {
  ...
  constructor(
    ...
    this.strategies.set(
      PaymentType.WIRE,
      new WireProcessingStrategy(wireGateway)
    );
    this.strategies.set(
      PaymentType.CHECK,
      new CheckProcessingStrategy(checkService)
    );
  }
...
export class PaymentStrategyFactory {
  ...
  async createCalculationStrategy(
    ...
    const preferredPaymentType = this.determinePreferredPaymentType(
      vendor,
      amount
    );
    ...
    switch (preferredPaymentType) {
      ...
      default:
        throw new Error(
          `Cannot determine payment strategy for vendor ${vendorId.getValue()}`
        );
    }
  }

  private determinePreferredPaymentType(
    vendor: Vendor,
    amount: Money
  ): PaymentType {
...
export class StandardLosProcessingStrategy implements ILosProcessingStrategy {
  ...
  async calculateAllocations(
    ...
    return await this.allocationService.calculateWorkingInterestAllocations(
      los
    );
  }
...
export class JointVentureLosProcessingStrategy
  implements ILosProcessingStrategy
{
  constructor(
...
export class LosProcessingService {
  private strategies: Map<LosProcessingType, ILosProcessingStrategy> =
    new Map();
  ...
  async processLos(los: LeaseOperatingStatement): Promise<LosProcessingResult> {
    ...
    return new LosProcessingResult(
      los.getId(),
      processingType,
      allocations,
      reports
...
describe('PaymentCalculationService', () => {
  ...
  it('should apply high value surcharge', async () => {
    ...
    expect(calculation.getRequiredApprovals()).toContain(
...
export class PaymentStrategyRegistry {
  ...
  register(
    paymentType: PaymentType,
    strategy: IPaymentCalculationStrategy
  ): void {
